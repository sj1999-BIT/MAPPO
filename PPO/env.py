...
class FinalEnv(Env):
    ...
    def run(self, controller: Controller, render=False, render_interval=1, debug=False):
        ...
        if debug:
            self.reset()
            envData = self.get_env_variables()
            while not self.renderer_controller.should_quit:
                if not controller.act(envData):
                    break
                ...
                self.print_end_effector_info()

                reward = controller.get_reward(envData)
                append_values_to_file(reward, filename="demo_reward_overtime.txt")





                # print(self.local_total_timesteps / 5)
            self.end_episode()
            self.print_stat()
            return
